...
def main
  @mruby = File.expand_path('../../bin/mruby', __FILE__)
  @verbose = false
  $stress = false
  dir = nil
  quiet = false
  ...
  ARGV.delete_if {|arg|
    ...
    when /\A--mruby=(.*)/
      @mruby = $1
      @mruby.gsub!(/^([^ ]*)/){File.expand_path($1)}
      @mruby.gsub!(/(\s+-I\s*)((?!(?:\.\/)*-(?:\s|\z))\S+)/){$1+File.expand_path($2)}
      @mruby.gsub!(/(\s+-r\s*)(\.\.?\/\S+)/){$1+File.expand_path($2)}
      true
    when /\A--sets=(.*)/
    ...
    when /\A(-h|--h(elp)?)\z/
      puts(<<-End)
Usage: #{File.basename($0, '.*')} --mruby=PATH [--sets=NAME,NAME,...]
...
End
  ...
  unless quiet
    ...
    puts "Target is #{`#{@mruby} -e '' --version`.chomp}"
    puts
    $stdout.flush
  end
...
def cleanup_coredump
  ...
  FileUtils.rm_f @mruby+'.stackdump' if @mruby
end
...
def check_coredump
  if File.file?('core') or not Dir.glob('core.*').empty? or
      (@mruby and File.exist?(@mruby+'.stackdump'))
