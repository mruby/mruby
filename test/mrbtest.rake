MRuby.each_target do
  ...
  file clib => [mrbcfile, init, asslib] + mrbs + csrc do |t|
    ...
    open(clib, 'w') do |f|
      ...
      csrc.each do |c|
        f.puts %Q[void mrb_init_#{File.basename(c, ".c")}_test(mrb_state *mrb);]
      end
      f.puts %Q[void mrbctest_init(mrb_state* mrb) {]
      csrc.each do |c|
        f.puts %Q[    mrb_init_#{File.basename(c, ".c")}_test(mrb);]
      end
      f.puts %Q[}]
