...
static const mrb_mt_entry range_rom_entries[] = {
  MRB_MT_ENTRY(range_beg,             MRB_SYM(begin),           MRB_MT_NOARG),
  MRB_MT_ENTRY(range_end,             MRB_SYM(end),             MRB_MT_NOARG),
  MRB_MT_ENTRY(range_eq,              MRB_OPSYM(eq),            0),
  MRB_MT_ENTRY(range_include,         MRB_OPSYM(eqq),           0),
  MRB_MT_ENTRY(range_excl,            MRB_SYM_Q(exclude_end),   MRB_MT_NOARG),
  MRB_MT_ENTRY(range_beg,             MRB_SYM(first),           MRB_MT_NOARG),
  MRB_MT_ENTRY(range_include,         MRB_SYM_Q(include),       0),
  MRB_MT_ENTRY(range_initialize,      MRB_SYM(initialize),      MRB_MT_PRIVATE),
  MRB_MT_ENTRY(range_end,             MRB_SYM(last),            MRB_MT_NOARG),
  MRB_MT_ENTRY(range_include,         MRB_SYM_Q(member),        0),
  MRB_MT_ENTRY(range_to_s,            MRB_SYM(to_s),            MRB_MT_NOARG),
  MRB_MT_ENTRY(range_inspect,         MRB_SYM(inspect),         MRB_MT_NOARG),
  MRB_MT_ENTRY(range_eql,             MRB_SYM_Q(eql),           0),
  MRB_MT_ENTRY(range_initialize_copy, MRB_SYM(initialize_copy), MRB_MT_PRIVATE),
  MRB_MT_ENTRY(range_num_to_a,        MRB_SYM(__num_to_a),      MRB_MT_NOARG),
};
