...
gen_call(codegen_scope *s, node *tree, mrb_sym name, int sp, int val, enum node_type nt)
{
  ...
  pop_n(n+1);
  {
    ...
    if (!noop && len == 1 && name[0] == '+')  {
      ...
      if (blk > 0) {                   /* no block */
        genop(s, MKOP_ABC(nt == NODE_FCALL? OP_FSEND : OP_SEND, cursp(), idx, n));
      }
      else {
        genop(s, MKOP_ABC(nt == NODE_FCALL? OP_FSENDB : OP_SENDB, cursp(), idx, n));
      }
    }
  }
  if (val) {
...
gen_assignment(codegen_scope *s, node *node, int sp, int val)
{
  ...
  case NODE_CALL:
    push();
    gen_call(s, node, attrsym(s, sym(node->cdr->car)), sp, NOVAL, NODE_CALL);
    pop();
    if (val) {
