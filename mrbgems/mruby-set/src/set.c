...
static const mrb_mt_entry set_rom_entries[] = {
  MRB_MT_ENTRY(set_size,              MRB_SYM(size),              MRB_MT_NOARG),
  MRB_MT_ENTRY(set_size,              MRB_SYM(length),            MRB_MT_NOARG),
  MRB_MT_ENTRY(set_empty_p,           MRB_SYM_Q(empty),           MRB_MT_NOARG),
  MRB_MT_ENTRY(set_clear,             MRB_SYM(clear),             MRB_MT_NOARG),
  MRB_MT_ENTRY(set_to_a,              MRB_SYM(to_a),              MRB_MT_NOARG),
  MRB_MT_ENTRY(set_include_p,         MRB_SYM_Q(include),         0),
  MRB_MT_ENTRY(set_include_p,         MRB_SYM_Q(member),          0),
  MRB_MT_ENTRY(set_include_p,         MRB_OPSYM(eqq),             0),
  MRB_MT_ENTRY(set_add,               MRB_SYM(add),               0),
  MRB_MT_ENTRY(set_add,               MRB_OPSYM(lshift),          0),
  MRB_MT_ENTRY(set_add_p,             MRB_SYM_Q(add),             0),
  MRB_MT_ENTRY(set_delete,            MRB_SYM(delete),            0),
  MRB_MT_ENTRY(set_delete_p,          MRB_SYM_Q(delete),          0),
  MRB_MT_ENTRY(set_init,              MRB_SYM(__init),            MRB_MT_NOARG),
  MRB_MT_ENTRY(set_core_merge,        MRB_SYM(__merge),           0),
  MRB_MT_ENTRY(set_core_subtract,     MRB_SYM(__subtract),        0),
  MRB_MT_ENTRY(set_core_union,        MRB_SYM(__union),           0),
  MRB_MT_ENTRY(set_core_difference,   MRB_SYM(__difference),      0),
  MRB_MT_ENTRY(set_core_intersection, MRB_SYM(__intersection),    0),
  MRB_MT_ENTRY(set_core_xor,          MRB_SYM(__xor),             0),
  MRB_MT_ENTRY(set_equal,             MRB_OPSYM(eq),              0),
  MRB_MT_ENTRY(set_hash_m,            MRB_SYM(hash),              MRB_MT_NOARG),
  MRB_MT_ENTRY(set_join,              MRB_SYM(join),              0),
  MRB_MT_ENTRY(set_inspect,           MRB_SYM(inspect),           MRB_MT_NOARG),
  MRB_MT_ENTRY(set_inspect,           MRB_SYM(to_s),              MRB_MT_NOARG),
  MRB_MT_ENTRY(set_reset,             MRB_SYM(reset),             MRB_MT_NOARG),
  MRB_MT_ENTRY(set_add_all,           MRB_SYM(add_all),           0),
  MRB_MT_ENTRY(set_delete_all,        MRB_SYM(delete_all),        0),
  MRB_MT_ENTRY(set_include_all_p,     MRB_SYM_Q(include_all),     0),
  MRB_MT_ENTRY(set_include_any_p,     MRB_SYM_Q(include_any),     0),
  MRB_MT_ENTRY(set_superset_p,        MRB_SYM_Q(superset),        0),
  MRB_MT_ENTRY(set_superset_p,        MRB_OPSYM(ge),              0),
  MRB_MT_ENTRY(set_proper_superset_p, MRB_SYM_Q(proper_superset), 0),
  MRB_MT_ENTRY(set_proper_superset_p, MRB_OPSYM(gt),              0),
  MRB_MT_ENTRY(set_subset_p,          MRB_SYM_Q(subset),          0),
  MRB_MT_ENTRY(set_subset_p,          MRB_OPSYM(le),              0),
  MRB_MT_ENTRY(set_proper_subset_p,   MRB_SYM_Q(proper_subset),   0),
  MRB_MT_ENTRY(set_proper_subset_p,   MRB_OPSYM(lt),              0),
  MRB_MT_ENTRY(set_intersect_p,       MRB_SYM_Q(intersect),       0),
  MRB_MT_ENTRY(set_disjoint_p,        MRB_SYM_Q(disjoint),        0),
  MRB_MT_ENTRY(set_cmp,               MRB_OPSYM(cmp),             0),
  MRB_MT_ENTRY(set_flatten,           MRB_SYM(flatten),           MRB_MT_NOARG),
  MRB_MT_ENTRY(set_flatten_bang,      MRB_SYM_B(flatten),         MRB_MT_NOARG),
};
