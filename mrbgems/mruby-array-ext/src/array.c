...
static const mrb_mt_entry array_ext_rom_entries[] = {
  MRB_MT_ENTRY(ary_assoc,             MRB_SYM(assoc),              0),
  MRB_MT_ENTRY(ary_at,                MRB_SYM(at),                 0),
  MRB_MT_ENTRY(ary_rassoc,            MRB_SYM(rassoc),             0),
  MRB_MT_ENTRY(ary_values_at,         MRB_SYM(values_at),          0),
  MRB_MT_ENTRY(ary_slice_bang,        MRB_SYM_B(slice),            0),
  MRB_MT_ENTRY(ary_compact,           MRB_SYM(compact),            MRB_MT_NOARG),
  MRB_MT_ENTRY(ary_compact_bang,      MRB_SYM_B(compact),          MRB_MT_NOARG),
  MRB_MT_ENTRY(ary_rotate,            MRB_SYM(rotate),             0),
  MRB_MT_ENTRY(ary_rotate_bang,       MRB_SYM_B(rotate),           0),
  MRB_MT_ENTRY(ary_sub,               MRB_OPSYM(sub),              0),
  MRB_MT_ENTRY(ary_difference,        MRB_SYM(difference),         0),
  MRB_MT_ENTRY(ary_union,             MRB_OPSYM(or),               0),
  MRB_MT_ENTRY(ary_union_multi,       MRB_SYM(union),              0),
  MRB_MT_ENTRY(ary_intersection,      MRB_OPSYM(and),              0),
  MRB_MT_ENTRY(ary_intersection_multi, MRB_SYM(intersection),      0),
  MRB_MT_ENTRY(ary_intersect_p,       MRB_SYM_Q(intersect),        0),
  MRB_MT_ENTRY(ary_fill_parse_arg,    MRB_SYM(__fill_parse_arg),   0),
  MRB_MT_ENTRY(ary_fill_exec,         MRB_SYM(__fill_exec),        0),
  MRB_MT_ENTRY(ary_uniq,              MRB_SYM(__uniq),             MRB_MT_NOARG),
  MRB_MT_ENTRY(ary_uniq_bang,         MRB_SYM_B(__uniq),           MRB_MT_NOARG),
  MRB_MT_ENTRY(ary_flatten,           MRB_SYM(flatten),            0),
  MRB_MT_ENTRY(ary_flatten_bang,      MRB_SYM_B(flatten),          0),
  MRB_MT_ENTRY(ary_normalize_index,   MRB_SYM(__normalize_index),  0),
  MRB_MT_ENTRY(ary_fetch,             MRB_SYM(__fetch),            0),
  MRB_MT_ENTRY(ary_insert,            MRB_SYM(insert),             0),
  MRB_MT_ENTRY(ary_deconstruct,       MRB_SYM(deconstruct),        MRB_MT_NOARG),
  MRB_MT_ENTRY(ary_product_group,     MRB_SYM(__product_group),    0),
  MRB_MT_ENTRY(ary_combination_init,  MRB_SYM(__combination_init), 0),
  MRB_MT_ENTRY(ary_combination_next,  MRB_SYM(__combination_next), 0),
};
