...
static void
mpz_init_capa(mpz_ctx_t *ctx, mpz_t *s, size_t hint)
{
  s->sn = 0;
  if (hint > 0) {
...
div_limb(mpz_ctx_t *ctx, mpz_t *q, mpz_t *r, mpz_t *x, mp_limb d)
{
  ...
  if ((d & (d - 1)) == 0) {
    ...
    if (shift == 0) {
      ...
      if (limb_shift >= x->sz) {
        ...
        mpz_init_capa(ctx, &temp_q, new_size);
        mpz_init(ctx, &temp_r);
  ...
  if (x->sz == 1) {
    ...
    mpz_init_capa(ctx, &temp_q, 1);
    mpz_init_capa(ctx, &temp_r, 1);
