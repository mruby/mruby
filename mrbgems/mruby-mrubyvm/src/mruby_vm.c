...
vm_stat(mrb_state *mrb, mrb_value cls)
{
  ...
  case MRB_TT_SYMBOL: {
    ...
    if (sym == mrb_intern_lit(mrb, "global_method_state")) {
      return mrb_fixnum_value(mrb_vm_global_method_state);
    } else if (sym == mrb_intern_lit(mrb, "global_constant_state")) {
      return mrb_fixnum_value(mrb_vm_global_constant_state);
    } else if (sym == mrb_intern_lit(mrb, "class_serial")) {
      return mrb_fixnum_value(mrb_vm_class_serial);
    }
  } break;

  case MRB_TT_HASH:
#define set_val(v) \
    mrb_hash_set(mrb, res, mrb_symbol_value(mrb_intern_lit(mrb, #v)), mrb_fixnum_value(mrb_vm_ ## v))
