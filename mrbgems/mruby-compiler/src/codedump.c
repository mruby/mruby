...
static const char*
local_name(mrc_ccontext *c, const mrc_irep *irep, size_t n)
{
  if (n == 0) return NULL;
  if (!irep->lv) return NULL;
  if (n >= irep->nlocals) return NULL;
  if (!irep->lv[n-1]) return NULL;
  const char *s = mrc_sym_dump(c, irep->lv[n-1]);
...
{
  const char *s = local_name(c, irep, n);
  if (s) fprintf(out, " R%d:%s", (int)n, s);
}

static void
print_lv_a(mrc_ccontext *c, const mrc_irep *irep, uint16_t a, FILE *out)
{
  if (local_name(c, irep, a)) {
    fprintf(out, "\t;");
...
{
  if (local_name(c, irep, a) || local_name(c, irep, b)) {
    fprintf(out, "\t;");
    print_r(c, irep, a, out);
    print_r(c, irep, b, out);
  }
  fprintf(out, "\n");
}
...
//  int ai;
  ...
  if (irep->lv) {
    ...
    for (int i = 1; i < irep->nlocals; i++) {
      char const *s = local_name(c, irep, i);
      if (s) {
        if (!head) {
          head = TRUE;
  ...
  while (pc < pcend) {
    ...
    CASE(OP_RANGE_EXC, B):
      ...
      break;
    CASE(OP_DEF, BB):
      fprintf(out, "DEF\t\tR%d\t:%s\t(R%d)\n", a, mrc_sym_dump(c, irep->syms[b]), a+1);
      break;
    CASE(OP_TDEF, BBB):
